...
async function getWBNBOHLCVData(poolAddress) {
    try {
        ...
        const response = await axios.get(`https://api.geckoterminal.com/api/v2/networks/${chain}/pools/${poolAddress}/ohlcv/${timeFrame}?aggregate=${aggregate}&limit=${limit}&token=${token}`)
...
const provider = envToBool(process.env.FORKING) ? hardhat.ethers.provider : new ethers.providers.JsonRpcProvider(process.env.URL)

const nonFungiblePositionManagerAddress = process.env.NONFUNGIBLEPOSITIONMANAGERADDRESS
const factoryAddress = process.env.FACTORYADDRESS


const Token_0 = new Token(parseInt(process.env.CHAIN_ID), process.env.TOA, parseInt(process.env.TOD), process.env.TOS, process.env.TON)
const Token_1 = new Token(parseInt(process.env.CHAIN_ID), process.env.T1A, parseInt(process.env.T1D), process.env.T1S, process.env.T1N)


const factoryContract = new hardhat.ethers.Contract(factoryAddress, IUniswapV3Factory, provider)
const nonfungiblePositionManagerContract = new hardhat.ethers.Contract(nonFungiblePositionManagerAddress, INonfungiblePositionManagerABI, provider)
const Token0_contract = new hardhat.ethers.Contract(Token_0.address, wethAbi, provider)
const Token1_contract = new hardhat.ethers.Contract(Token_1.address, uniAbi, provider,)
const quoterContract = new ethers.Contract(process.env.UNISWAPV3QUOTER, IQuoterABI, provider)
const routerContract = new ethers.Contract(process.env.UNISWAPV3ROUTER, ISwapRouterABI, provider)

const amount0 = hardhat.ethers.utils.parseUnits(process.env.LIQUIDITY_AMOUNT, 18).toString()
...
const getPoolData = async (poolContract) => {
    ...
    return { tickSpacing: tickSpacing, fee: fee, liquidity: liquidity, sqrtPriceX96: slot0[0], tick: slot0[1] }
}
...
const createPosition = async (signer) => {
    try {
        const poolContract = await getPoolContract(Token_1.address, Token_0.address, '3000')

        logger.info(`Address of pool ${poolContract.address}`)
        const poolData = await getPoolData(poolContract)
        const pool = new Pool(
            Token_1, Token_0, poolData.fee,
        ...
        //!------new code------------------------------------------------------------------
        const tickLower = nearestTick - (poolData.tickSpacing * parseInt(multVolatility))
        const tickUpper = nearestTick + (poolData.tickSpacing * parseInt(multVolatility))
        ...
        const { amount0: amountRequired0, amount1: amountRequired1 } = positionObj.mintAmounts
        ...
        const allowance1 = await Token1_contract.connect(signer).allowance(Token_1.address, signer.address)
        if (allowance1.lt(amount0)) {
            logger.info(`Allowance is only ${allowance1.toString()} while need ${amount0}`)
            // throw false
            await Token1_contract.connect(signer).approve(nonFungiblePositionManagerAddress, amount0)
        }

        const allowance0 = await Token0_contract.connect(signer).allowance(Token_0.address, signer.address)
        if (allowance0.lt(amount1)) {
            logger.info(`Allowance is only ${allowance0.toString()} while need ${amount1}`)
            await Token0_contract.connect(signer).approve(nonFungiblePositionManagerAddress, amount1)
        }
        ...
        await sleep(1000)
        logger.info("\n=======> Adding position....")
        const tx = await nonfungiblePositionManagerContract.connect(signer).mint(params, { gasLimit })
...
const getTokenId = async (receipt) => {
    const nfpmContractAddress = process.env.NONFUNGIBLEPOSITIONMANAGERADDRESS.toLowerCase()
    const contractInterface = new hardhat.ethers.utils.Interface(INonfungiblePositionManagerABI)
    const transferTopic = contractInterface.getEventTopic('Transfer')
    const filteredLogs = receipt.logs.filter(log => log.address.toLowerCase() === nfpmContractAddress && log.topics.includes(transferTopic))
...
const getTokenQuantities = async (tokenId) => {
    ...
    const token0contract = new hardhat.ethers.Contract(position.token0, Token_0.address === position.token0 ? wethAbi : uniAbi, provider)
    const token1contract = new hardhat.ethers.Contract(position.token1, Token_0.address === position.token1 ? wethAbi : uniAbi, provider)
    ...
    const poolContract = await getPoolContract(position.token0, position.token1, position.fee)
    ...
    const sqrtPrice = sqrtPriceX96 / JSBI.exponentiate(JSBI.BigInt(2), JSBI.BigInt(96))
...
const getWalletBalances = async (position, signer) => {
    ...
    const balance0String = `${symbol0}: ${hardhat.ethers.utils.formatUnits(balance0.toString(), decimals0)}`
    const balance1String = `${symbol1}: ${hardhat.ethers.utils.formatUnits(balance1.toString(), decimals1)}`
...
const leveragePosition = async (signer, tokenId, tokenToBorrow) => {
    try {
        ...
        const tx = await nonfungiblePositionManagerContract.connect(signer)['safeTransferFrom(address,address,uint256,bytes)'](signer.address, process.env.UNISWAPV3LEVERAGEADDRESS, tokenId, encodedData, { gasLimit })
        const receipt = await tx.wait()
...
const deleveragePosition = async (signer, positionContractAddress) => {
    try {
        const positionContract = new ethers.Contract(positionContractAddress, positionAbi, signer)
        const gasLimit = hardhat.ethers.utils.hexlify(9000000)
        logger.info("\n=======> Removing leverage....")
        const tx = await positionContract.deleverage(process.env.FLASHLOANPROVIDER, {
        ...
        logger.info(`Got error while try to deleverage position : ${error.message}`)
        return false
    }
}

const getPositionAddressFromReceipt = async (receipt) => {
    const uniswapV3LeverageContractAddress = process.env.UNISWAPV3LEVERAGEADDRESS.toLowerCase()

    const contractInterface = new hardhat.ethers.utils.Interface(uniswapV3LeverageABI)

    const leveragedPositionCreatedTopic = contractInterface.getEventTopic('LeveragedPositionCreated')
    ...
    if (filteredLogs.length > 0) {
        const positionAddress = ethers.utils.defaultAbiCoder.decode(['address'], filteredLogs[0].topics[1])[0]
        logger.info(`Position Address: ${positionAddress}`)
        return positionAddress
    } else {
        console.error('No LeveragedPositionCreated event found in the transaction receipt.')
        return null
    }
}
...
async function loadWallet(address) {
    ...
    const signer = await hardhat.ethers.getImpersonatedSigner(process.env.IMPERSONATED_ACCOUNT)
    const gasLimit = hardhat.ethers.utils.hexlify(350000)

    await signer.sendTransaction({ to: process.env.WETHWHALE, value: ethers.utils.parseUnits("1", 18) }, { gasLimit })

    const WETH_WHALE = await hardhat.ethers.getImpersonatedSigner(process.env.WETHWHALE)
    const WETH = new hardhat.ethers.Contract(Token_1.address, wethAbi, provider)
    logger.info(`Balance of account before transfer: ${ethers.utils.formatUnits(await WETH.balanceOf(signer.address), 18)}`)
    await WETH.connect(WETH_WHALE).transfer(address, ethers.utils.parseUnits("1", 18))
    logger.info(`Balance of account after transfer: ${ethers.utils.formatUnits(await WETH.balanceOf(signer.address), 18)}`)
}

const checkPositionInRange = async (tokenId) => {
    try {
        // Fetch the position details
        const positionDetails = await nonfungiblePositionManagerContract.positions(tokenId)
        const poolContract = await getPoolContract(positionDetails.token0, positionDetails.token1, positionDetails.fee)
        const slot0 = await poolContract.slot0()
        ...
        logger.info("==============================================================")
        logger.info(`Position ${tokenId} is ${isInRange ? 'in' : 'out of'} range. `)
        logger.info("==============================================================")
        if (isInRange) {
...
async function swapTokensUniswap(signer, amountIn, tokenInAddress, tokenOutAddress, amountOutMin) {
    // Approve the router to spend the token
    const tokenInContract = new ethers.Contract(tokenInAddress, uniAbi, signer)

    const allowance = await tokenInContract.connect(signer).allowance(tokenInAddress, signer.address)
...
const rebalanceToThresholdRatio = async (signer) => {
    try {
        ...
        const adjustedBalanceToken0 = balanceToken0.mul(ethers.utils.parseUnits("1", 18)).div(ethers.utils.parseUnits("1", decimalsToken0))
        const adjustedBalanceToken1 = balanceToken1.mul(ethers.utils.parseUnits("1", 18)).div(ethers.utils.parseUnits("1", decimalsToken1))

        // Get the current price of Token0 in terms of Token1
        const amountOutToken1ForToken0 = await quoterContract.callStatic.quoteExactInputSingle(
            Token_0.address,
            Token_1.address,
        ...
        const totalValueInToken1Terms = adjustedBalanceToken1.add(adjustedBalanceToken0.mul(amountOutToken1ForToken0).div(ethers.utils.parseUnits("1", decimalsToken0)))

        // Calculate percentage of total value for each token
        const percentageToken0 = adjustedBalanceToken0.mul(amountOutToken1ForToken0).div(ethers.utils.parseUnits("1", decimalsToken0)).mul(100).div(totalValueInToken1Terms)
        const percentageToken1 = adjustedBalanceToken1.mul(100).div(totalValueInToken1Terms)
        ...
        if (percentageToken1.lt(lowerThreshold) || percentageToken1.gt(upperThreshold)) {
            if (percentageToken1.lt(lowerThreshold)) {
                ...
                const amountToken0ToSwap = desiredBalanceToken1.sub(adjustedBalanceToken1).mul(ethers.utils.parseUnits("1", decimalsToken0)).div(amountOutToken1ForToken0)

                // Calculate minimum amount out to handle 5% slippage
                const amountOutMin = amountOutToken1ForToken0.mul(95).div(100) // 5% slippage

                logger.info(`Swapping ${ethers.utils.formatUnits(amountToken0ToSwap, decimalsToken0)} ${Token_0.symbol} to ${Token_1.symbol}`)
                // Implement swap logic here
                if (envToBool(process.env.ONE_INCH_SWAP)) {
                    logger.info(`======== 1inch =========== from: ${Token_1.address} ===== to: ${Token_0.address} ==== amount: ${amountToken0ToSwap}`)

                    await swapOn1inch(signer, Token_0.address, Token_1.address, amountToken0ToSwap.toString())
                } else {
                    await swapTokensUniswap(signer, amountToken0ToSwap.toString(), Token_0.address, Token_1.address, amountOutMin.toString())
                }
            } else if (percentageToken1.gt(upperThreshold)) {
                // Token1 is more than 55%, meaning Token0 is less than 45%, swap Token1 for Token0
                const desiredBalanceToken0 = totalValueInToken1Terms.div(2).div(amountOutToken1ForToken0).mul(ethers.utils.parseUnits("1", decimalsToken0))
                const amountToken1ToSwap = adjustedBalanceToken1.sub(desiredBalanceToken0.mul(amountOutToken1ForToken0).div(ethers.utils.parseUnits("1", decimalsToken0)))

                // Since we're swapping Token1 to Token0, calculate the expected Token0 amount for the Token1 amount to swap
                const expectedToken0Amount = await quoterContract.callStatic.quoteExactInputSingle(
                    Token_1.address,
                    Token_0.address,
                    3000,
                    amountToken1ToSwap,
                ...
                logger.info(`Swapping ${ethers.utils.formatUnits(amountToken1ToSwap, decimalsToken1)} ${Token_1.symbol} to ${Token_0.symbol}`)
                // Implement swap logic here
                if (envToBool(process.env.ONE_INCH_SWAP)) {
                    logger.info(`======== 1inch =========== from: ${Token_1.address} ===== to: ${Token_0.address} ==== amount: ${amountToken1ToSwap}`)
                    await swapOn1inch(signer, Token_1.address, Token_0.address, amountToken1ToSwap.toString())
                } else {
                    await swapTokensUniswap(signer, amountToken1ToSwap.toString(), Token_1.address, Token_0.address, amountOutMinToken0.toString())
                }
            }
        } else {
            logger.info("Wallet balance is within the desired threshold. No rebalancing needed.")
        }
        return true
    } catch (error) {
...
const swapOn1inch = async (signer, baseToken, dest_token, amount) => {
    ...
    if (tokenAllowance.lt(amount)) {
        logger.info(`Allowance is only ${tokenAllowance.toString()} while need ${amount}`)
    ...
    const temp_swap_tx = await swap1inch(baseToken, dest_token, amount, signer.address, 5)
...
async function LeveragePositionWithTries(signer, tokenId, tokenAddress) {
    ...
    do {
        const leverage_receipt = await leveragePosition(signer, tokenId, tokenAddress)
        if (leverage_receipt) {
            positionAddress = await getPositionAddressFromReceipt(leverage_receipt)
        }
        if (!positionAddress) { counter++ }
...
async function MonitorPositionWithTries(tokenId) {
    ...
    do {
        ...
        if (monitoringResult === null) { counter++ }
...
async function DecreaseLiquidityWithTries(signer, params) {
    ...
    do {
        try {
            receipt = await nonfungiblePositionManagerContract.connect(signer).decreaseLiquidity(params, { gasLimit: '1000000' })
        } catch (error) {
...
async function CollectWithTries(signer, params) {
    ...
    do {
        try {
            receipt = await nonfungiblePositionManagerContract.connect(signer).collect(params, { gasLimit: '1000000' })
        } catch (error) {
    ...
    while (errorCounter < 3) {
        try {
            ...
            if (envToBool(process.env.LEVERAGE)) {
                positionAddress = await LeveragePositionWithTries(signer, tokenId, process.env.T1A)
            }
            ...
            if (positionAddress) {
                ...
                do {
                    ...
                    if (envToBool(process.env.FORKING)) {
                        await performLargeSwap(Token_0.address, Token_1.address, ethers.utils.parseEther("120000"))
                    }
                    await sleep(waitTime * 1000) // Wait for 30 seconds before the next check
                    logger.info(`LiquidityPosition is ${monitoringResult ? 'still in' : 'out of'} range. Waiting for next check after ${waitTime} sec ...\n`)
                } while (monitoringResult && outOfRange < 2)

                if (envToBool(process.env.LEVERAGE)) {
                    const deleverage = await DeleveragePositionWithTries(signer, positionAddress)
                    if (!deleverage) {
            ...
            const [amount0weiBefore, amount1weiBefore] = await getTokenQuantities(tokenId)

            const currentLiquidity = position.liquidity
            const decreasePercent = 1
            const decreasePercentShiftedBN = hardhat.ethers.BigNumber.from(decreasePercent * 100)
            const decreaseLiquidityAmountShiftedBN = currentLiquidity.mul(decreasePercentShiftedBN)
            const decreaseLiquidity = decreaseLiquidityAmountShiftedBN.div(hardhat.ethers.BigNumber.from(100))
            ...
            const [amount0weiAfter, amount1weiAfter] = await getTokenQuantities(tokenId)

            const token0Change = hardhat.ethers.BigNumber.from((amount0weiBefore - amount0weiAfter).toString())
            const token1Change = hardhat.ethers.BigNumber.from((amount1weiBefore - amount1weiAfter).toString())
            ...
            if (e.message.includes('Not enough') || e.message.includes('insufficient funds')) {
                errorCounter++
            }
...
main().then(() => logger.info("Bot stopped")).catch(error => console.error("Bot encountered a startup error:", error))